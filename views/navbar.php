...
<head>
    ...
    <style>
    /* small helper for hiding scrollbar if you use .scrollbar-hidden */
...
<body class="bg-gray-50">
    ...
    <div class="w-full flex items-center justify-between px-6 py-6 bg-gray-100 shadow">
        ...
        <div id="menu_toggle" class="flex-1 xl:hidden lg:hidden block">
        ...
        <div id="menu_sidebar" class="fixed inset-0 bg-black/50 h-full w-full z-20 hidden">
            <div
                ...
                <ul class="flex flex-col gap-4 text-md text-gray-500 mt-6 uppercase w-full">
                    ...
                    <li class="hover:text-[#ff4e00] cursor-pointer border-b border-gray-300 p-2">
                        <i class="fa-solid fa-magnifying-glass pe-2"></i>Search
                    </li>
                    <li class="hover:text-[#ff4e00] cursor-pointer border-b border-gray-300 p-2">
                        <i class="fa-regular fa-user pe-2"></i>Login/Register
                    </li>
                </ul>
            </div>
        </div> <!-- end menu_sidebar -->
        ...
        <!-- Right side icons -->
        <div class="flex-1 flex justify-end space-x-4 text-xl items-center">

            <!-- Search -->
            <button class="search_bar"><i class="fa-solid fa-magnifying-glass cursor-pointer"></i></button>
            ...
            <!-- Login/user page include -->
            <?php include('./loginUser/login-register.php'); ?>

            <!-- Wishlist -->
            <button class="wishlist_bar"><i class="fa-regular fa-heart cursor-pointer text-xl"></i></button>
            <div id="wishlist_sidebar" class="fixed inset-0 bg-black/50 h-full w-full z-20 left-0 top-0 hidden">
                <div
                    class="absolute right-0 flex flex-col items-center bg-white h-full w-80 p-5 transform translate-x-full transition-transform duration-300 ease-in-out">
            ...
            <!-- Cart -->
            <button class="cart_bar relative">
                <i class="fa-solid fa-cart-shopping cursor-pointer text-xl"></i>
                <!-- Badge -->
                <?php
// If you are storing cart items in session
$total_items = 0;
if (isset($_SESSION['cart'])) {
    // Assuming $_SESSION['cart'] is an array of items
    foreach ($_SESSION['cart'] as $item) {
        $total_items += $item['qty']; // or just count 1 per item if quantity not used
    }
}
?>
                <span
            ...
            <div id="cart_sidebar" class="fixed inset-0 bg-black/50 h-full w-full z-20 left-0 top-0 hidden">
                <div
                    class="absolute right-0 flex flex-col bg-white h-full w-96 p-5 transform translate-x-full transition-transform duration-300 ease-in-out shadow-lg">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-lg font-semibold">Your Cart</h2>
                        <button class="close_cart text-gray-600 hover:text-red-600">✕</button>
                    </div>

                    <div id="cart-container" class="flex-1 overflow-y-auto scrollbar-hidden">
                        <?php include './cart/nav_cart.php' ?>
                    </div>
                </div>
            </div>

        </div> <!-- end right side icons -->
            ...
            <div id="sale-container" class="relative dropdown">
                ...
                <div id="sale-dropdown"
                    ...
                    <div class="w-[50%] relatives">
                        ...
                        <span class="absolute top-[45%] text-white left-[10%] font-semibold">New Arrival</span>
                    </div>
    ...
    $(function() {
        ...
        $('.search_bar').click(function() {
            ...
            $('#search_sidebar .search_coming, #search_sidebar .search_coming')
        ...
        $(".cart_bar").click(function() {
            $('#cart_sidebar').removeClass('hidden'); // show overlay
            $('#cart_sidebar > div')
                .removeClass('translate-x-full')
                .addClass('translate-x-0');
            $('body').addClass('overflow-hidden'); // disable main page scroll
        });
        ...
        $(document).on('click', '#cart_sidebar', function(e) {
            if (e.target.id === 'cart_sidebar') {
                $('#cart_sidebar > div')
                    ...
                    .addClass('translate-x-full');
                setTimeout(() => {
                    $('#cart_sidebar').addClass('hidden'); // hide overlay
                    $('body').removeClass('overflow-hidden'); // restore scroll
                }, 300);
            }
        });

        // Close Cart (with a close button inside cart)
        ...
        // small dropdown toggle for mobile menu
        ...
        $(document).ready(function() {
            ...
            for (let i = 0; i < totalSlides; i++) {
                $dots.append(
                    `<span class="dot w-3 h-3 rounded-full border border-gray-400 inline-block cursor-pointer ${i === 0 ? 'bg-black' : 'bg-white'}"></span>`
                );
            }
            ...
            function updateSlider() {
                ...
                // Update dots
                $("#dots .dot").removeClass("bg-black").addClass("bg-white");
                $("#dots .dot").eq(currentIndex).removeClass("bg-white").addClass("bg-black");
            }
        ...
        $(document).ready(function() {
            ...
            $(document).on("click", ".plus-btn", function() {
                ...
                $.post("./cart/cart_action.php", {
                    id: id,
                    action: "increase"
                }, function(resp) {
                    $("#nav_cart").html(resp); // update cart HTML
                    updateCartBadge();
                });
            });
            ...
            function updateCartBadge() {
                $.get("./cart/cart_count.php", function(count) {
                    $(".cart_bar span").text(count); // update total items
                });
            }
